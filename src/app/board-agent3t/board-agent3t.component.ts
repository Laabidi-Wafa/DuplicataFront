...
export class BoardAgent3tComponent implements OnInit {
  ...
  constructor(private userService: UserService ,private modalService: NgbModal,
    private auth: AuthService , 
     private route: ActivatedRoute, private demandeservise: DemandeService, 
    ...
    this.demande = this.demandeservise.getMesDemandes();
     this.demandeservise.getMesDemandes().subscribe(
      data => {
        ...
        for ( var i = 0 ; i < this.offre.length; i++ ) { if ( this.offre[i].etat === 'NonValider') {
          this.showNotif = true; console.log(this.showNotif) }
    }
      },
      err => {
...
reloadfishe() {
 ...
 console.log(this.demandeFiche);
  this.demandeservise.getMesDemandesf().subscribe(
      data => {
        ...
        for ( var i=0 ; i < this.offre.length ; i++ ) {
         ...
         { console.log( 'test',this.offre[i].etat) ;
          this.shownotifpaix = true ;   console.log(this.shownotifpaix);}
         }
      }
);
}
...
affId() {
   this.auth.affierid().subscribe(
      data => {
        ...
        this.test = {'id':'1' , 'username': 'yasmine', 'prenom':'amara' , 'date':'13-02-2020' 
        ,'email':'amara14@gmail.' ,'password':'ghddks' ,'sexe':'m' ,'tel':'21332415','role_id':'1'};
      },
      err => {
        this.content = JSON.parse(err.error).message;
      }
